...
#    5. Asks for WiFi SSID and password → patches src/main.rs
#    6. Builds the project in release mode
#    7. Detects the USB serial port and flashes the device
# =============================================================================
set -euo pipefail
...
# ── 6. Serial port permissions (dialout group) ────────────────────────────────
step "Checking serial port permissions..."

if ! groups "$USER" | grep -q '\bdialout\b'; then
    warn "User '$USER' is not in the 'dialout' group."
    warn "Adding to dialout group — you may need to log out and back in for this to take effect."
...
# Show current values from main.rs
...
# Fall back to current values if user just pressed Enter
...
# Use Python for safe substitution — handles special chars in SSID/password
...
content = re.sub(
    r'const WIFI_SSID:\s+&str\s*=\s*"[^"]*";',
    f'const WIFI_SSID:   &str = "{ssid}";',
    content
)
content = re.sub(
    r'const WIFI_PASS:\s+&str\s*=\s*"[^"]*";',
    f'const WIFI_PASS:   &str = "{password}";',
    content
)
